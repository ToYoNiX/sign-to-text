...
def extract(landmarks) -> np.ndarray:
    ...
    for mcp_i, pip_i, tip_i in FINGERS:
        ...
        directions.extend((d_vec / n).tolist() if n > 1e-6 else [0., 0., 0.])

        # Curl = 1 − cos(angle at PIP)   0 = straight, ~2 = fully bent
        v1 = mcp - pip;  n1 = np.linalg.norm(v1)
        v2 = tip - pip;  n2 = np.linalg.norm(v2)
        if n1 > 1e-6 and n2 > 1e-6:
            cos_a = float(np.clip(np.dot(v1 / n1, v2 / n2), -1., 1.))
            curls.append(1. - cos_a)
        else:
            curls.append(0.)
    ...
    palm_n  = (normal / n_norm).tolist() if n_norm > 1e-6 else [0., 0., 0.]
