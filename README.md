...
dataset/              filtered + mirrored samples ready for training (gitignored)
  quality/            per-label variance heatmaps (PNG), generated by build_dataset.py
...
fix_mirrorable.py     dry-run / fix mirrorable flags for directional signs in raw/
...
features.py           feature extraction — 86-float vector from 21 landmarks (shared by all scripts)
...
**(Optional) Fix mirrorable flags** — directional signs (ر، ز، و، etc.) should have `mirrorable: false`. Run this if you've added new raw captures and are unsure:
```bash
python fix_mirrorable.py          # dry run — shows what would change
python fix_mirrorable.py --apply  # write changes

poetry run python fix_mirrorable.py
poetry run python fix_mirrorable.py --apply
```
...
```
Outputs curated samples to `dataset/` and per-label variance heatmaps to `dataset/quality/`.
...
2. (Optional) Fix mirrorable flags for any directional signs:
  ```bash
  python fix_mirrorable.py --apply
  ```
3. Build the curated dataset:
  ```bash
  python build_dataset.py --keep 30
  ```
4. Train models (with or without augmentation):
  ```bash
  python train.py                 # default augment (15x)
  python train.py --augment 25    # custom augment
  # or: python train.py --no-augment
  ```
5. Export the SVM to ONNX (for the static site):
  ```bash
  python export_onnx.py
  ```
6. Build the static site (optional; GitHub Actions does this on push):
  ```bash
  python build.py
  ```
...
```bash
python train.py && python export_onnx.py
# or
poetry run python train.py && poetry run python export_onnx.py
