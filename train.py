...
def augment_sample(x: np.ndarray, n: int = 15) -> list:
    ...
    for _ in range(n):
        lm = lm_orig.copy()
        if np.random.rand() > 0.4:  lm = _noise(lm)
        if np.random.rand() > 0.5:  lm = _rotate(lm)
        if np.random.rand() > 0.6:  lm = _flip_depth(lm)
        lm = _scale(lm)
...
def train(augment_copies: int = 15):
    ...
    svm_cv = cross_val_score(
        ...
        X_real_sc, y_real_enc, cv=cv, scoring="accuracy"
    )
    ...
    plot_confusion(y_train, le.inverse_transform(svm.predict(X_scaled)),
                   classes_str, MODELS_DIR / "svm_confusion.png")
    plot_confusion(y_train, le.inverse_transform(rf.predict(X_scaled)),
                   classes_str, MODELS_DIR / "rf_confusion.png")
    print("\nAll done.")
